...
send_data(struct http_request *req, int fd)
{
	...
	int datas;
	const char *type;
	if ((datas = read(fd, buf, BUFFSIZE)) < 0)
		panic("send_data:failed to read"); 
	if (datas > BUFFSIZE)
		panic("buffer too small!");

	if (write(req->sock, buf, datas) != datas)
		return -1;
...
send_file(struct http_request *req)
{
	...
	struct Stat stat;
	if( (r = fstat(fd, &stat,0)) < 0 )
	  panic("httpd.c: fstat wrong %e\n",r);
	if(stat.st_isdir){
		send_error(req,404);
		goto end;
	}
	...
	if ((r = send_header_fin(req)) < 0)
		goto end;
	cprintf("already have a client and want to send\n");
	r = send_data(req, fd);
