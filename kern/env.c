...
env_alloc(struct Env **newenv_store, envid_t parent_id)
{
	...
	*newenv_store = e;
	...
	//inherit the uid and git from parent
	envindex = ENVX(parent_id);
	if( envindex >  0 && envindex < NENV ){
		e->ruid = envs[envindex].ruid;
		e->euid = envs[envindex].euid;
		e->rgid = envs[envindex].rgid;
		e->egid = envs[envindex].egid;
