	   ...
	   void **pg_store, int *perm_store)
{
	...
	// Open the file
	if (req->req_omode & (O_CREAT | O_MKDEV | O_MKDIR)) {
		rwxu = 0;
		rwxg = 0;
		rwxo = 0;
		...
		if(req->req_omode & O_MKDEV){
			f->f_type = FTYPE_DEV;		//set the file type to dev
			rwxu = F_READABLE | F_WRITABLE | F_EXECUTAB;
			rwxg = F_READABLE | F_WRITABLE ;
			rwxo = F_READABLE | F_WRITABLE ;
		}
		else if(req->req_omode & O_MKDIR){
			f->f_type = FTYPE_DIR;
			rwxu = F_READABLE | F_WRITABLE | F_EXECUTAB;
			rwxg = F_READABLE | F_EXECUTAB;
			rwxo = F_READABLE | F_EXECUTAB;
		}
		else{
			rwxu = F_READABLE | F_WRITABLE ;
			rwxg = F_READABLE | F_WRITABLE ;
			rwxo = F_READABLE ;
...
try_open:
	...
	//PERMISSION CHECK	not match rwx return -E and pg null  else return 0
	ugo = 0;		//user group other select
	checkrwx = 0;	//checkrwx initialized to 0
	euid = req->req_euid;
	egid = req->req_egid; 

	// Chmod before authority
	if (req->req_omode & O_CHMOD) {
		if (euid != 0 && euid != f->fuid) {
			return -E_PRERM_DENIED;
		}
	}

	//cprintf("rwx:%02x\teuid:%d\n",req->checkrwx,euid);
	
	r = (req->req_omode & O_ACCMODE); //r is just a tmp 
	if(r == O_RDWR)
		checkrwx = checkrwx | F_READABLE | F_WRITABLE;
	else if(r == O_RDONLY)
		checkrwx = checkrwx | F_READABLE;
	else if(r == O_WRONLY)
		checkrwx = checkrwx | F_WRITABLE;

	if(req->req_omode & O_EXE)
		checkrwx = checkrwx | F_EXECUTAB;

	if(euid != 0){			//if root need not check
	...
	//Append
	if(req->req_omode & O_APPEN){
		o->o_fd->fd_offset = f->f_size;
	}
	...
	if(f->f_type == FTYPE_DEV){
		o->o_fd->fd_dev_id = devcons.dev_id;
	}
	else{
...
serve_stat(envid_t envid, union Fsipc *ipc)
{
	...
	//CHMOD change rwx
	if(req->change)
		if(euid == 0 || euid == o->o_file->fuid)
		{
			memmove(o->o_file->rwx,req->changerwx,3);
		}
	...
	ret->ret_uid = o->o_file->fuid;
	ret->ret_gid = o->o_file->fgid;
	strcpy(ret->ret_rwx,o->o_file->rwx);
