...
// readline.c
char*	readline(const char *buf,char* bufout,int buflen);
...
int sys_getruid(envid_t envid);
int sys_geteuid(envid_t envid);
int sys_getrgid(envid_t envid);
int sys_getegid(envid_t envid);
...
int	fstat(int fd, struct Stat *statbuf,char ifchange);
int	stat(const char *path, struct Stat *statbuf,char ifchange);
...
#define O_MKDEV		0X1000		/* create device file */
#define O_APPEN		0X2000		/* append buf to file */
#define O_EXE		0X4000		/* open as executable file*/
#define O_CHMOD		0X8000		/* open for chmod rename chmod...*/
