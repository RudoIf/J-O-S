...
spawn(const char *prog, const char **argv)
{
	...
	if ((r = open(prog, O_RDONLY | O_EXE)) < 0)
		return r;
	fd = r;
...
copy_shared_pages(envid_t child)
{
	...
	{
		//I make use of a "if" attribute here as well as in fork.
		//Thus, I can combine following 3 confition together.
		//"if" will check from left to right in C 
		//and decide whether neccessary to check next according to the expression.
		//So pgfaukt will not happen here.
		if(	(vpd[PDX(addr)] & PTE_P) > 0 && ( vpt[PGNUM(addr)] & PTE_P) > 0 && ( vpt[PGNUM(addr)] & PTE_U) > 0)
		{
			pte = vpt[PGNUM(addr)];
			perm = pte & PTE_SYSCALL;	//USE PTE_SYSCALL to mask
			if (perm & PTE_SHARE)
			{
				if ((r = sys_page_map (0, (void*)addr, child, (void*)addr, PTE_U|PTE_P|PTE_W|PTE_SHARE)) < 0)
