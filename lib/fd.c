...
int
fstat(int fdnum, struct Stat *stat,char rwx)
{
	int r;
	struct Dev *dev;
	struct Fd *fd;
	...
	stat->st_isdir = 0;
	stat->st_dev = dev;
	return (*dev->dev_stat)(fd, stat,rwx);
}

int
stat(const char *path, struct Stat *stat,char ifchange)
{
	int fd, r;
	int perm = O_RDONLY;
	if(ifchange)
		perm = O_CHMOD;
	if ((fd = open(path, perm)) < 0)
		return fd;
	r = fstat(fd, stat, ifchange);
	close(fd);
	return r;
}
